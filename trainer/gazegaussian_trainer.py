        ...
        for i, data in enumerate(data_loader):
            if start_frame == i:
                ...
                with torch.set_grad_enabled(False):
                    ...
                    # 创建保存重定向图片的文件夹
                    save_dir = "results/redirected_images"
                    os.makedirs(save_dir, exist_ok=True)

                    # 提取渲染出的完整头部图像
                    img_to_save = novel_gaze_head_set
                    if isinstance(img_to_save, list):
                        img_to_save = img_to_save[0]  # 取第一张

                    # 如果是 tensor，转成 numpy
                    if torch.is_tensor(img_to_save):
                        img_to_save = img_to_save.squeeze().cpu().numpy()
                        if img_to_save.shape[0] == 3:  # CHW to HWC
                            img_to_save = img_to_save.transpose(1, 2, 0)
                        img_to_save = (img_to_save * 255.0).astype(np.uint8)

                    # OpenCV 需要 BGR 格式，进行转换并保存
                    if isinstance(img_to_save, np.ndarray):
                        # 如果数组已经是 0-255 的 uint8 就不需要乘 255 了
                        ...
                        # print(f"图片已保存: {save_path}") # 可选：打印保存路径
                    return
